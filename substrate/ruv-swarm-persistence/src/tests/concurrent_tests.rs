...
async fn test_task_assignment_race_conditions() {
    ...
    assert!(
        max_claims - min_claims <= 3,
        "Task distribution should be roughly fair"
    );
}
...
async fn test_event_sourcing_at_scale() {
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        ...
        println!(
            "SQLite Performance: {} events in {:?} ({:.0} events/sec)",
        ...
        if recent_events[i - 1].timestamp > recent_events[i].timestamp {
            ...
            if i >= 2 {
                println!(
                    "Event {}: id={}, timestamp={}",
                    i - 2,
                    recent_events[i - 2].id,
                    recent_events[i - 2].timestamp
                );
            }
...
async fn test_metric_aggregation_performance() {
            ...
            for batch in 0..(metrics_per_agent / batch_size) {
                for i in (batch * batch_size)..((batch + 1) * batch_size) {
                    let metric = MetricModel {
                        ...
                        tags: [("host".to_string(), format!("host-{}", idx % 10))]
                            .into_iter()
                            .collect(),
    ...
    let aggregated = storage
        .get_aggregated_metrics("cpu_usage", start_time.timestamp(), Utc::now().timestamp())
        .await
        .unwrap();
    let query_duration = query_start.elapsed();
    ...
    assert!(
        ...
        "Aggregation query took too long: {:?}",
        query_duration
    );
