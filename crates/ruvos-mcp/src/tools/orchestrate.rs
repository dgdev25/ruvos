...
/// Returns the explicit list of MCP tool operations the host should execute after
/// running inference for this archetype.  Each entry is a `ToolOp` object with
/// `op`, `description`, and optional `args` that Claude Code passes to
/// `ruvos_agent_exec`.  Archetypes that only produce text (planner, reviewer,
/// coordinator) return an empty list.
fn archetype_tool_plan(archetype: &str) -> serde_json::Value {
    use serde_json::json;
    match archetype {
        "coder" => json!([
            { "op": "write_file",   "description": "Write the generated implementation to the target path", "args": { "path": "<inferred from task>", "content": "<inference output>" } },
            { "op": "run_command",  "description": "Run the test suite to verify the implementation", "args": { "cmd": "cargo test", "args": [] } },
        ]),
        "tester" => json!([
            { "op": "write_file",   "description": "Write the generated test file to the target path", "args": { "path": "<inferred from task>", "content": "<inference output>" } },
            { "op": "run_command",  "description": "Run tests to confirm RED (failing) state before implementation", "args": { "cmd": "cargo test", "args": [] } },
        ]),
        "devops" => json!([
            { "op": "run_command",  "description": "Execute the CI/CD or deployment command", "args": { "cmd": "<inferred from task>", "args": [] } },
            { "op": "git_op",       "description": "Commit any generated config or pipeline files", "args": { "op": "commit", "message": "chore: generated by devops agent" } },
        ]),
        "data" => json!([
            { "op": "write_file",   "description": "Write the generated migration or schema file", "args": { "path": "<inferred from task>", "content": "<inference output>" } },
            { "op": "run_command",  "description": "Run migrations or schema validation", "args": { "cmd": "<inferred from task>", "args": [] } },
        ]),
        "docs" => json!([
            { "op": "write_file",   "description": "Write the generated documentation file", "args": { "path": "<inferred from task>", "content": "<inference output>" } },
        ]),
        "security" => json!([
            { "op": "run_command",  "description": "Grep codebase for identified vulnerability patterns", "args": { "cmd": "grep", "args": ["-rn", "<pattern>", "src/"] } },
        ]),
        "researcher" => json!([
            { "op": "run_command",  "description": "Search codebase for relevant symbols or patterns", "args": { "cmd": "grep", "args": ["-rn", "<query>", "."] } },
        ]),
        // planner, reviewer, architect, perf, coordinator — inference only, no file ops
        _ => json!([]),
    }
}
            ...
            let coordinator_steps: Vec<Value> = pipeline
                ...
                .map(|(index, archetype)| {
                    let role_prefix = swarm_step_roles
                        ...
                        .unwrap_or_else(|| format!("[{} orchestration] {}", label, task));
                    let tool_plan = archetype_tool_plan(archetype);
                    json!({
                        "step_index": index,
                        "archetype": archetype,
                        ...
                        "inject_via": format!(
                            "Call ruvos_agent_spawn with archetype={archetype} and the inference result as prompt; \
                             then execute tool_plan ops via ruvos_agent_exec"
                        ),
                        "tool_plan": tool_plan,
                    })
                })
