...
impl ToolHandler for HooksPreHandler {
    ...
    fn validate(&self, params: &Value) -> Result<()> {
        ...
        let obj = params.as_object().ok_or_else(|| {
            crate::RufloError::ValidationError("params must be an object".to_string())
        })?;
    ...
    fn execute(&self, params: Value) -> ExecuteFuture {
        ...
        Box::pin(async move {
            let obj = params.as_object().ok_or_else(|| {
                crate::RufloError::InvalidParams("params must be an object".to_string())
            })?;

            let kind_str = obj.get("kind").and_then(|v| v.as_str()).ok_or_else(|| {
                crate::RufloError::InvalidParams("kind must be a string".to_string())
            })?;
            ...
            let hook_kind = match kind_str {
                ...
                _ => {
                    return Err(crate::RufloError::InvalidParams(format!(
                        "invalid hook kind: {}",
                        kind_str
                    )))
                }
            ...
            let response = dispatcher
                ...
                .await
                .map_err(|e| crate::RufloError::InternalError(e.to_string()))?;
    ...
    fn execute(&self, params: Value) -> ExecuteFuture {
        ...
        Box::pin(async move {
            ...
            let success = obj
                ...
                .ok_or_else(|| {
                    crate::RufloError::InvalidParams("success must be a boolean".to_string())
                })?;
