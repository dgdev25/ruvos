...
fn arb_task_model(existing_agents: Vec<String>) -> impl Strategy<Value = TaskModel> {
    ...
    (
        ...
        if agent_count == 0 {
            ...
            (0..agent_count)
                .prop_map(move |idx| Some(agents[idx].clone()))
                .boxed()
        },
        0..1000usize,
        0i64..3600,
        ...
        .prop_map(
            |(id, description, priority, status, assigned_to, data_size, created_offset)| {
                let now = Utc::now();
                let created_at = now - Duration::seconds(created_offset);
...
fn arb_storage_op(existing_agents: Vec<String>) -> impl Strategy<Value = StorageOp> {
    ...
    if agent_count == 0 {
        ...
        prop_oneof![
            ...
            // ClaimTask - 20% chance
            (arb_task_model(existing_agents.clone()), 0..agent_count).prop_map(
                move |(task, idx)| StorageOp::ClaimTask {
                    task_id: task.id,
...
async fn check_storage_invariants(
    ...
    let agent_ids: HashSet<_> = storage
        .list_agents()
        .await
        .unwrap()
        .into_iter()
