...
impl AgentRegistry {
    ...
    fn enqueue_message(&mut self, agent_id: &str, content: String) -> Result<String> {
        ...
        if let Some(queue) = self.message_queues.get_mut(agent_id) {
            ...
            return Err(crate::RufloError::ValidationError(format!(
                "Agent not found: {}",
                agent_id
            )));
        }
...
impl ToolHandler for AgentSpawnHandler {
    ...
    fn validate(&self, params: &Value) -> Result<()> {
        ...
        params
            .get("archetype")
            .and_then(|v| v.as_str())
            .ok_or_else(|| crate::RufloError::ValidationError("Missing: archetype".into()))?;

        params
            .get("prompt")
            .and_then(|v| v.as_str())
            .ok_or_else(|| crate::RufloError::ValidationError("Missing: prompt".into()))?;

        params
            .get("model")
            .and_then(|v| v.as_str())
            .ok_or_else(|| crate::RufloError::ValidationError("Missing: model".into()))?;
        ...
        if let Some(budget) = params.get("budget").and_then(|v| v.as_u64()) {
            if budget == 0 {
                return Err(crate::RufloError::ValidationError(
    ...
    fn execute(&self, params: Value) -> ExecuteFuture {
        Box::pin(async move {
            ...
            if let Some(ref mut r) = *reg {
                ...
                Err(crate::RufloError::InternalError(
            ...
            if let Some(ref r) = *reg {
                if let Some(id) = agent_id {
                    ...
                    if let Some(agent) = r.get_agent(id) {
                        ...
                        Err(crate::RufloError::ValidationError(format!(
                            "Agent not found: {}",
                            id
                        )))
                    }
...
impl ToolHandler for AgentMessageHandler {
    ...
    fn validate(&self, params: &Value) -> Result<()> {
        params
            .get("agent_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| crate::RufloError::ValidationError("Missing: agent_id".into()))?;

        params
            .get("message")
            .and_then(|v| v.as_str())
            .ok_or_else(|| crate::RufloError::ValidationError("Missing: message".into()))?;
