...
fn eval_compress_can_compare_against_saved_report() {
    ...
    let baseline = Command::new(env!("CARGO_BIN_EXE_ruvos"))
        .args(["eval", "compress", "--write", baseline_path.to_str().expect("utf8 path")])
        .output()
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_ruvos"))
        .args(["eval", "compress", "--compare-to", baseline_path.to_str().expect("utf8 path")])
        .output()
...
fn eval_orchestrate_handoff_write_and_compare() {
    ...
    let write_out = Command::new(env!("CARGO_BIN_EXE_ruvos"))
        .args(["eval", "orchestrate-handoff", "--write", baseline_path.to_str().unwrap()])
        .output()
    ...
    let cmp_out = Command::new(env!("CARGO_BIN_EXE_ruvos"))
        .args(["eval", "orchestrate-handoff", "--compare-to", baseline_path.to_str().unwrap()])
        .output()
        .expect("compare");
...
fn eval_swarm_recovery_write_and_compare() {
    ...
    let write_out = Command::new(env!("CARGO_BIN_EXE_ruvos"))
        .args(["eval", "swarm-recovery", "--write", baseline_path.to_str().unwrap()])
        .output()
        .expect("write baseline");
    assert!(write_out.status.success(), "write failed");
    assert!(baseline_path.exists());
    ...
    let cmp_out = Command::new(env!("CARGO_BIN_EXE_ruvos"))
        .args(["eval", "swarm-recovery", "--compare-to", baseline_path.to_str().unwrap()])
        .output()
        .expect("compare");
    assert!(cmp_out.status.success(), "compare failed");
...
fn eval_skill_routing_write_and_compare() {
    ...
    let write_out = Command::new(env!("CARGO_BIN_EXE_ruvos"))
        .args(["eval", "skill-routing", "--write", baseline_path.to_str().unwrap()])
        .output()
        .expect("write baseline");
    assert!(write_out.status.success(), "write failed");
    assert!(baseline_path.exists());
    ...
    let cmp_out = Command::new(env!("CARGO_BIN_EXE_ruvos"))
        .args(["eval", "skill-routing", "--compare-to", baseline_path.to_str().unwrap()])
        .output()
        .expect("compare");
    assert!(cmp_out.status.success(), "compare failed");
...
fn eval_swarm_learning_write_and_compare() {
    ...
    let write_out = Command::new(env!("CARGO_BIN_EXE_ruvos"))
        .args(["eval", "swarm-learning", "--write", baseline_path.to_str().unwrap()])
        .output()
        .expect("write baseline");
    assert!(write_out.status.success(), "write failed");
    assert!(baseline_path.exists());
    ...
    let cmp_out = Command::new(env!("CARGO_BIN_EXE_ruvos"))
        .args(["eval", "swarm-learning", "--compare-to", baseline_path.to_str().unwrap()])
        .output()
        .expect("compare");
    assert!(cmp_out.status.success(), "compare failed");
